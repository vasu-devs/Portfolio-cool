...
<div class="contact-directory"><section aria-labelledby="reach-title"><h2 id="reach-title">Get in touch</h2><ul>${['Email','LinkedIn','Book a call'].map(contactLink).join('')}</ul><div class="directory-copy-email"><a href="mailto:${escape(resume.email)}">${escape(resume.email)}</a><button type="button" class="copy-mail" data-copy="${escape(resume.email)}">Copy email</button></div></section><section aria-labelledby="elsewhere-title"><h2 id="elsewhere-title">Elsewhere</h2><ul>${['GitHub','YouTube','X','Instagram','Résumé'].map(contactLink).join('')}</ul></section></div>
