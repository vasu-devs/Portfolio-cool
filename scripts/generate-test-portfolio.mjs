...
const techMark=name=>{const spec=techIcons[name];let mark,cls='tech';if(!spec)mark=`<i class="tech-letter" aria-hidden="true">${escape(name.slice(0,2))}</i>`;else if(Array.isArray(spec)){mark=siSvg(spec[1],16);cls+=' tech--mono';}else mark=deviconSvg(spec);return `<li class="${cls}" data-tip="${escape(name)}" tabindex="0">${mark}<span class="sr">${escape(name)}</span></li>`;};
