...
const darkCoverUpdates = new Set(["Vaani", "Odeon", "BranchGPT", "JustHireMe"]);
const coverFor=(name,realm)=>`/test/covers/${name.toLowerCase()}-${realm}-v${realm === "hueco" && darkCoverUpdates.has(name) ? 3 : 2}.webp`;
...
const projectMarkup = projectCards.filter((_,i)=>!hobbyProjects.has(projects[i].name)).join('');
const hobbyMarkup = projectCards.filter((_,i)=>hobbyProjects.has(projects[i].name)).join('');
const roleMarkup = roles.map(r => `<details class="role"><summary><h3>${escape(r.company)}</h3><p class="role-name">${escape(r.role)}</p><span class="role-date mono">${escape(r.dateLabel)}</span><p class="role-preview">${escape(r.contribution)}</p><span class="role-hint">Read about this role <span aria-hidden="true">+</span></span></summary><div class="role-body"><p>${escape(r.summary)}</p>${r.sections ? r.sections.map(section).join('') : ''}</div></details>`).join('');
...
const archiveCards = names => names.map(name=>archive.find(p=>p.name===name)).filter(Boolean).map(p=>`<article class="archive-item" data-work-category="${escape(p.category)}"><div class="archive-item-heading"><h3><button type="button" class="archive-open" aria-haspopup="dialog">${escape(p.name)}</button></h3><span>${escape(p.category)}</span></div><p>${escape(p.summary)}</p>${p.details?.length?`<details class="archive-detail"><summary>Read case study <span aria-hidden="true"></span></summary><p class="archive-study-summary">${escape(p.summary)}</p><p class="case-label">${escape(p.status)}</p><div class="case-stack">${p.tech.map(t=>`<span>${escape(t)}</span>`).join('')}</div><div class="case-sections">${(originalFor(p.name)?.details||p.details).map(section).join('')}</div>${p.url?`<a class="text-link" href="${escape(p.url)}" target="_blank" rel="noopener noreferrer">Explore the repository</a>`:''}${p.demo?`<a class="text-link" href="${escape(p.demo)}" target="_blank" rel="noopener noreferrer">Watch the demo</a>`:''}</details>`:''}</article>`).join('');
const replacements = {
 CONTACTOPTIONS: socials.filter(s=>['Email','LinkedIn','X','Book a call'].includes(s.label)).map(s=>`<a href="${escape(s.url)}" ${s.url.startsWith('https')?'target="_blank" rel="noopener noreferrer"':''}>${socialIcon(s.label)}<span>${escape(s.label)}</span></a>`).join(''),
  ...
  CONTACTS: (()=>{
    const link=s=>`<a href="${escape(s.url)}" data-social="${escape(s.label.toLowerCase().replaceAll(' ','-'))}" aria-label="${escape(s.label)}" title="${escape(s.label)}"${s.url.startsWith('mailto:')?'':' target="_blank" rel="noopener noreferrer"'}>${socialIcon(s.label)}<span class="${['Email','LinkedIn','Résumé'].includes(s.label)?'contact-label':'sr-only'}">${escape(s.label)}</span></a>`;
    return `<div class="primary-contacts">${socials.filter(s=>['Email','LinkedIn','Résumé'].includes(s.label)).map(link).join('')}</div><details class="secondary-contacts"><summary>Elsewhere</summary><div>${socials.filter(s=>!['Email','LinkedIn','Résumé'].includes(s.label)).map(link).join('')}</div></details>`;
  })(),
