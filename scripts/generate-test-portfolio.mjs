...
const replacements = {
  ...
  CONTACTS: socials.map(s=>`<a href="${escape(s.url)}" aria-label="${escape(s.label)}" title="${escape(s.label)}"${s.url.startsWith('mailto:')?'':' target="_blank" rel="noopener noreferrer"'}>${socialIcon(s.label)}<span class="sr-only">${escape(s.label)}</span></a>`).join(''),
  VIDEOS: videos.map(v=>{
    const p=projects.find(p=>p.name===v.name)||catalog.find(p=>p.name===v.name)||archive.find(p=>p.name===v.name);
    const id=new URL(v.url).searchParams.get('v');
    return `<a class="video-link" href="${escape(v.url)}" aria-haspopup="dialog"><span class="video-play" aria-hidden="true">▷</span><span><strong>${escape(v.name)}</strong><small>${escape(v.summary)}</small></span><span aria-hidden="true">↗</span></a><template id="film-${id}"><h2>${escape(v.name)}</h2><p class="film-summary">${escape(p?.summary||v.summary)}</p>${p?.tech?`<div class="case-stack">${p.tech.map(t=>`<span>${escape(t)}</span>`).join('')}</div>`:''}<div class="case-sections">${(p?.details||[]).map(section).join('')}</div>${p?.url?`<a class="text-link" href="${escape(p.url)}" target="_blank" rel="noopener noreferrer">Explore the repository ↗</a>`:''}</template>`;
  }).join(''),
