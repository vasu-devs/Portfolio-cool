...
const replacements = {
 ...
 CONTACTOPTIONS: ['Email','Book a call','LinkedIn'].map(label=>socials.find(s=>s.label===label)).filter(Boolean).map(s=>`<a href="${escape(s.url)}" ${s.url.startsWith('https')?'target="_blank" rel="noopener noreferrer"':''}>${socialIcon(s.label)}<span><strong>${escape(s.label==='Email'?'Email me':s.label==='LinkedIn'?'Message on LinkedIn':s.label)}</strong><small>${escape(s.label==='Email'?resume.email:s.label==='Book a call'?'Choose a time on my calendar':'Connect or send me a message')}</small></span><span class="contact-arrow" aria-hidden="true">&#8599;</span></a>`).join(''),
  ...
  CONTACTS: (()=>{
    ...
    return `<div class="social-icon-row">${['X','GitHub','LinkedIn','Email','YouTube'].map(label=>socials.find(s=>s.label===label)).filter(Boolean).map(link).join('')}</div><details class="secondary-contacts"><summary>More links</summary><div>${socials.filter(s=>!['X','GitHub','LinkedIn','Email','YouTube'].includes(s.label)).map(link).join('')}</div></details>`;
  })(),
