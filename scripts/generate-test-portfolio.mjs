...
import { Mail, Github, Linkedin, Youtube, Instagram, CalendarDays, FileText, BookOpen, Play, ExternalLink } from 'lucide-react';
...
import {projects as content, groups, pageCopy, archiveNames, coverPath, skills} from './test-portfolio-content.mjs';
...
const iconAction=(tag,attrs,Icon,label)=>`<${tag} ${attrs} class="icon-action" aria-label="${escape(label)}" title="${escape(label)}">${glyph(Icon)}<span class="icon-action-text">${escape(label)}</span></${tag}>`;
const actions = p => `<div class="proj-actions">${p.sections.length ? iconAction('button','type="button" data-open aria-haspopup="dialog"',BookOpen,'Case study') : ''}${p.url ? iconAction('a',`href="${escape(p.url)}" target="_blank" rel="noopener noreferrer"`,Github,'Source on GitHub') : ''}${p.demo ? iconAction('a',`href="${escape(p.demo)}" aria-haspopup="dialog"`,Play,'Watch walkthrough') : ''}${p.live ? iconAction('a',`href="${escape(p.live)}" target="_blank" rel="noopener noreferrer"`,ExternalLink,'Open live site') : ''}</div>`;
...
const replacements = {
 ...
 CONTACTOPTIONS: ['Email','Book a call','LinkedIn'].map(label=>socials.find(s=>s.label===label)).filter(Boolean).map(s=>`<a href="${escape(s.url)}" ${s.url.startsWith('https')?'target="_blank" rel="noopener noreferrer"':''}>${socialIcon(s.label)}<span><strong>${escape(s.label==='Email'?'Email me':s.label==='LinkedIn'?'Message on LinkedIn':s.label)}</strong><small>${escape(s.label==='Email'?resume.email:s.label==='Book a call'?'Choose a time on my calendar':'Connect or send me a message')}</small></span></a>`).join(''),
  ...
  SELECTED: ['justhireme','branchgpt','odeon'].map(slug=>{
    const p=bySlug(slug);
    return `<article class="selected-item"><a class="selected-art featured-preview" href="#project-${p.slug}" aria-label="Read ${escape(p.name)} case study">${covers(p, `${p.name} product preview`)}</a><div class="selected-copy"><p class="proj-kicker">${escape(p.kind)}</p><h3><a href="#project-${p.slug}">${escape(p.name)}</a></h3><p class="selected-summary">${escape(p.deck)}</p><p class="proj-stack">${p.tech.slice(0,5).map(escape).join(' · ')}</p><div class="proj-actions">${iconAction('a',`href="#project-${p.slug}"`,BookOpen,'Case study')}${p.url?iconAction('a',`href="${escape(p.url)}" target="_blank" rel="noopener noreferrer"`,Github,'Source on GitHub'):''}${p.demo?iconAction('a',`href="${escape(p.demo)}" aria-haspopup="dialog"`,Play,'Watch walkthrough'):''}</div></div></article>`;
  }).join(''),
