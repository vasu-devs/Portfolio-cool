...
document.addEventListener('keydown', event => {
 ...
 if (document.querySelector('dialog[open],.bleach-menu[open]')) return;
 if (!enabled.checked) return;
 const key = event.key.toLowerCase();
 const pages = {h:'home',w:'projects',a:'about',c:'contact'};
 let action;
 if (pages[key]) action = () => { location.hash = pages[key]; };
 else if (key === 't') action = () => document.querySelector('.theme-toggle').click();
