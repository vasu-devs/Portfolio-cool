...
export function mountBleachDetails(controls){
 ...
 const extras=document.createElement('details');extras.className='character-extras';const extraSummary=document.createElement('summary');extraSummary.textContent='Discoveries & references';extras.append(extraSummary);panel.append(extras);const heading=document.createElement('h3');heading.textContent='Discoveries';list=document.createElement('ul');status=document.createElement('p');status.setAttribute('role','status');status.className='bleach-status';extras.append(heading,list,status);
